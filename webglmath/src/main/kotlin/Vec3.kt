...
class Vec3(backingStorage: Float32Array?, offset: Int = 0) : UniformFloat {
  ...
  fun rotateZ(angle: Float): Vec3 {
    ...
    val x = storage[0]
    val y = storage[1]
    storage[0] = x * cos - y * sin
    storage[1] = x * sin + y * cos
    return this
  }
  ...
  fun rotateY(angle: Float): Vec3 {
      ...
      val x = storage[0]
      val z = storage[2]
      storage[0] = x * cos + z * sin
      storage[2] = -x * sin + z * cos
      return this
  }
  ...
  fun rotateX(angle: Float): Vec3 {
      ...
      val y = storage[1]
      val z = storage[2]
      storage[1] = y * cos - z * sin
      storage[2] = y * sin + z * cos
      return this
  }

  fun lerp(other: Vec3, lerping: Float): Vec3 {
    storage[0] = storage[0] * (1 - lerping) + other.storage[0] * lerping
    storage[1] = storage[1] * (1 - lerping) + other.storage[1] * lerping
    storage[2] = storage[2] * (1 - lerping) + other.storage[2] * lerping
    return this
  }

  fun distance(center: Vec3): Float {
    return sqrt(
      (storage[0] - center.storage[0]) * (storage[0] - center.storage[0]) +
      (storage[1] - center.storage[1]) * (storage[1] - center.storage[1]) +
      (storage[2] - center.storage[2]) * (storage[2] - center.storage[2])
    )
  }

  fun reflect(normal: Vec3): Vec3 {
    val dot = this dot normal
    storage[0] = storage[0] - 2 * dot * normal.storage[0]
    storage[1] = storage[1] - 2 * dot * normal.storage[1]
    storage[2] = storage[2] - 2 * dot * normal.storage[2]
    return this
  }
}
